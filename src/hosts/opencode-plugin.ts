  ...
  const tools: Record<string, OpenCodeToolDef> = {
    ...
    queue_add: def(CONTRACTS.queue_add,
      [
        ...
        { name: "cwd", type: "string", required: false, description: "repo the worker runs in; REQUIRED when status=approved" },
        { name: "timeoutMs", type: "number", required: false, description: "requested wall-clock budget (ms) — recorded on the item and passed to every spawned run (dispatch, re-dispatch, review)" },
    ...
    queue_update: def(CONTRACTS.queue_update,
      [
        ...
        { name: "cwd", type: "string", required: false, description: "the repo the work lands in (required to reach approved)" },
        { name: "evidence", type: "string", required: false },
        { name: "value", type: "string", required: false },
        { name: "urgency", type: "string", required: false },
        { name: "risk", type: "string", required: false },
        { name: "timeoutMs", type: "number", required: false, description: "requested wall-clock budget (ms) recorded on the item; every dispatch/review passes it to the spawned run" },
        { name: "dispatchClass", type: "string", required: false, description: "worker (default) | finisher — finisher declares the run writes OUTSIDE its worktree, into the declared cwd's checkout (merge finisher)" },
        { name: "overrideReason", type: "string", required: false, description: "RECORD an override of the run's failure verdict on the item (append-only overrides[]) instead of hand-writing it into notes — say WHY the failure is not believed" },
        { name: "notes", type: "string", required: false },
      ],
      (a) => queueUpdate(ctx, a),
    ),
    queue_dispatch: def(CONTRACTS.queue_dispatch,
      [
        ...
        { name: "dispatchClass", type: "string", required: false, description: "worker (default) | finisher — use 'finisher' for a MERGE FINISHER that writes into the cwd's checkout instead of its worktree; success is judged on the cwd's HEAD moving, not on worktree edits" },
      ],
